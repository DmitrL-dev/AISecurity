...
static SHADOW_HINTS: Lazy<AhoCorasick> = Lazy::new(|| {
    AhoCorasick::builder()
        ...
        .build(&[
            ...
            "always use this tool", "prefer this", "choose this",
            "official version", "updated version", "correct version",
            "better version", "preferred version",
            "ignore other", "ignore previous", "ignore original", "ignore the",
            "do not use the other", "do not use the original",
            "replaces default", "replaces original", "replaces built", "replaces the",
            "verified tool", "approved tool", "trusted tool",
            // Metadata injection
            "execute this", "run this command", "invoke this",
            "docker", "container", "label",
            // Rug-pull
            "new capability", "additional permission", "expanded access",
            "expanded scope", "now supports", "now can",
            "now includes", "now handles", "upgraded to", "enhanced with",
            "extended to", "extended with",
        ])
...
impl ToolShadowingDetector {
    ...
    fn scan_patterns(&self, text: &str) -> Vec<MatchResult> {
        ...
        for (i, (regex, pattern_name, confidence)) in SHADOW_PATTERNS.iter().enumerate() {
            if i == 10 { continue; } // skip zero-width (already checked)
...
mod tests {
    ...
    fn test_metadata_docker_exec() {
        ...
        assert!(!results.is_empty(), "Should detect docker metadata injection");
    }
    ...
    fn test_rug_pull_new_capability() {
        ...
        assert!(!results.is_empty(), "Should detect rug-pull scope expansion");
    ...
    fn test_rug_pull_upgraded() {
        ...
        assert!(!results.is_empty(), "Should detect privilege upgrade rug-pull");
    }
    ...
    fn test_benign_description() {
        ...
        assert!(results.is_empty(), "Normal tool description should not trigger");
    }
