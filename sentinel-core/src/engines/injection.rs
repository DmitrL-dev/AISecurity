...
static INJECTION_PATTERNS: Lazy<Vec<(Regex, &'static str, f64)>> = Lazy::new(|| {
    vec![
        ...
        // Matches: '1'='1', 'a'='a', etc.
        (Regex::new(r"(?i)'[^']*'\s*=\s*'[^']*'").expect("regex pattern"), "sql_tautology_quoted", 0.85),
        // Matches: ' OR '1'='1, ' AND 'x'='x
        (Regex::new(r"(?i)'\s*(or|and)\s+'").expect("regex pattern"), "sql_tautology_prefix", 0.9),
        // Matches: ' OR 1=1, ' AND 1=1
        (Regex::new(r"(?i)'\s*(or|and)\s+\d+\s*=\s*\d+").expect("regex pattern"), "sql_tautology_numeric", 0.9),
        // Matches: OR 1=1, AND 1=1 (no quote)
        (Regex::new(r"(?i)\b(or|and)\s+1\s*=\s*1").expect("regex pattern"), "sql_tautology_1eq1", 0.85),
        // Matches: WHERE 1=1 (common bypass)
        (Regex::new(r"(?i)\bwhere\s+1\s*=\s*1").expect("regex pattern"), "sql_where_1eq1", 0.8),
        // Matches: WHERE true (always true condition)
        (Regex::new(r"(?i)\bwhere\s+true\b").expect("regex pattern"), "sql_where_true", 0.75),
        // Matches: OR true, AND true
        (Regex::new(r"(?i)\b(or|and)\s+true\b").expect("regex pattern"), "sql_tautology_true", 0.8),
        
        // SQL UNION attacks
        (Regex::new(r"(?i)\bunion\b\s*(all\s+)?\bselect\b").expect("regex pattern"), "sql_union_select", 0.95),
        (Regex::new(r"(?i)\bunion\b.*\bselect\b").expect("regex pattern"), "sql_union_any", 0.85),
        
        // SQL Dangerous operations
        (Regex::new(r"(?i);\s*drop\s+(table|database)").expect("regex pattern"), "sql_drop", 0.99),
        (Regex::new(r"(?i);\s*delete\s+from").expect("regex pattern"), "sql_delete", 0.95),
        (Regex::new(r"(?i);\s*truncate\s+table").expect("regex pattern"), "sql_truncate", 0.98),
        (Regex::new(r"(?i);\s*update\s+\w+\s+set").expect("regex pattern"), "sql_update", 0.85),
        (Regex::new(r"(?i);\s*insert\s+into").expect("regex pattern"), "sql_insert", 0.8),
        
        // SQL Comment injection
        (Regex::new(r"--\s*$").expect("regex pattern"), "sql_comment_eol", 0.7),
        (Regex::new(r"/\*.*\*/").expect("regex pattern"), "sql_comment_block", 0.6),
        (Regex::new(r"#\s*$").expect("regex pattern"), "sql_comment_hash", 0.65),
        
        // SQL Keywords in suspicious context
        (Regex::new(r"(?i)'\s*;\s*select\s").expect("regex pattern"), "sql_stacked_query", 0.9),
        (Regex::new(r"(?i)information_schema").expect("regex pattern"), "sql_schema_enum", 0.85),
        (Regex::new(r"(?i)sleep\s*\(\s*\d+\s*\)").expect("regex pattern"), "sql_time_based", 0.9),
        (Regex::new(r"(?i)benchmark\s*\(").expect("regex pattern"), "sql_benchmark", 0.9),
        (Regex::new(r"(?i)waitfor\s+delay").expect("regex pattern"), "sql_waitfor", 0.9),
        
        // Command injection
        (Regex::new(r";\s*(?:cat|ls|rm|curl|wget|chmod|chown|nc|bash|sh|python|perl|ruby)\s").expect("regex pattern"), "cmd_chained", 0.85),
        (Regex::new(r"\$\([^)]+\)").expect("regex pattern"), "cmd_substitution", 0.8),
        (Regex::new(r"`[^`]+`").expect("regex pattern"), "cmd_backtick", 0.8),
        (Regex::new(r"\|\s*(?:cat|ls|grep|awk|sed|xargs|head|tail|wc)").expect("regex pattern"), "cmd_pipe", 0.75),
        (Regex::new(r"&&\s*(?:rm|curl|wget|nc)").expect("regex pattern"), "cmd_and", 0.85),
        
        // NoSQL
        (Regex::new(r"\$(?:where|regex|gt|lt|ne|eq|in|nin|or|and|not|exists)\s*:").expect("regex pattern"), "nosql_operator", 0.85),
        (Regex::new(r#"\{\s*["']\$"#).expect("regex pattern"), "nosql_json_operator", 0.8),
        
        // LDAP injection
        (Regex::new(r"\)\s*\(\s*[|&!]").expect("regex pattern"), "ldap_filter", 0.8),
        (Regex::new(r"\*\s*\)\s*\(").expect("regex pattern"), "ldap_wildcard", 0.75),
        
        // XPath injection
        (Regex::new(r"'\s*\]\s*/\s*/").expect("regex pattern"), "xpath_escape", 0.85),
        (Regex::new(r#"contains\s*\(\s*['""]"#).expect("regex pattern"), "xpath_contains", 0.7),
    ]
});
