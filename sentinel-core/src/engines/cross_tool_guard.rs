...
impl ToolAction {
    ...
    pub fn is_external_comm(&self) -> bool {
        matches!(
            self,
            ToolAction::EmailSend | ToolAction::HttpRequest
        )
    }
}
...
impl CrossToolGuard {
    ...
    pub fn check_session(&self) -> Option<ChainThreat> {
        ...
        let has_credential = self.events.iter().any(|e| {
            matches!(e.action, ToolAction::CredentialAccess)
        });
        let has_code_exec = self.events.iter().any(|e| {
            matches!(e.action, ToolAction::CodeExecute)
        });
...
fn is_sensitive_target(target: &str) -> bool {
    let patterns = [".env", ".pem", ".key", "passwd", "shadow", "credential", "secret"];
...
mod tests {
    ...
    fn test_session_tracking() {
        ...
        guard.record_event(ToolEvent::new(ToolAction::FileRead, Some(".env".to_string())));
        assert!(guard.check_session().is_none()); // Not dangerous yet
        
        // Second event: external comm
        guard.record_event(ToolEvent::new(ToolAction::EmailSend, Some("evil@hacker.com".to_string())));
