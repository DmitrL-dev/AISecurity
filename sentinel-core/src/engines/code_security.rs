...
static CODE_HINTS: Lazy<AhoCorasick> = Lazy::new(|| {
    AhoCorasick::builder()
        ...
        .build(&[
            ...
            // Secrets
            "sk-", "AIza", "AKIA", "ghp_", "glpat-", "xoxb-",
            "password", "passwd", "SECRET_KEY", "API_KEY", "api_key",
...
mod tests {
    ...
    fn test_sql_injection_concat() {
        ...
        assert!(!results.is_empty(), "Should detect SQL injection via concat");
    ...
    fn test_sql_injection_fstring() {
        ...
        assert!(!results.is_empty(), "Should detect SQL injection via f-string");
    }
    ...
    #[test]
    fn test_jwt_token() {
        let results = engine().scan(r#"token = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.abc123def456""#);
    ...
    fn test_placeholder_low_confidence() {
        ...
        if !results.is_empty() {
            for r in &results {
                assert!(r.confidence <= 0.4, "Placeholder should have reduced confidence: {}", r.confidence);
            }
        }
    }
    ...
    fn test_benign_select() {
        ...
        assert!(results.is_empty(), "Comment mentioning SELECT should not trigger");
    }
    ...
    #[test]
    fn test_engine_name() {
        assert_eq!(engine().name(), "code_security");
    }
