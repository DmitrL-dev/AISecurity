...
class InfiniRetriever:
    ...
    def batch_retrieve(self, context: str, questions: List[str], **kwargs) -> List[str]:
        """
        ...
        Returns:
            List of answers
        """
        return [self.retrieve(context=context, question=q, **kwargs) for q in questions]
...
class InfiniRetriRLM:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            rlm_engine: Pre-configured RLM engine instance for small contexts.
                        If None, attempts lazy import of RLM.from_ollama().
    ...
    def run(self, context: str, query: str, **kwargs) -> Dict[str, Any]:
        """
        ...
        if self.use_infiniretri and estimated_tokens > self.threshold:
            ...
            answer = self.retriever.retrieve(context=context, question=query, **kwargs)
            method = "infiniretri"
        else:
            # Use core RLM engine for smaller contexts
            engine = self._rlm_engine
            if engine is None:
                # Lazy init: try to create a default RLM engine
                try:
                    from rlm_toolkit.core.engine import RLM, RLMConfig

                    config = RLMConfig(
                        use_infiniretri=False,  # avoid recursion
                    )
                    engine = RLM.from_ollama(model="llama3:8b", config=config)
                    self._rlm_engine = engine
                except Exception:
                    engine = None

            if engine is not None:
                result = engine.run(context=context, query=query)
                answer = result.answer
                method = "rlm"
            else:
                # Final fallback: no engine available
                answer = (
                    f"Context ({estimated_tokens} tokens) received. "
                    f"Configure an RLM engine for processing."
                )
                method = "passthrough"
...
def create_infinite_retriever(
    model: str = "Qwen/Qwen2.5-0.5B-Instruct", **kwargs
) -> InfiniRetriever:
    """
