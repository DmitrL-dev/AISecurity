# Session tools — v1 bridge (MemoryBridgeManager)
"""
Tools: start_session, sync_state, restore_state,
       auto_context, get_state, update_goals,
       record_decision, add_hypothesis, add_fact,
       search_facts, build_communities,
       list_sessions, delete_session, get_audit_log
"""
from __future__ import annotations

from typing import Any, Dict, List, Optional

from ._common import ServerType
...
def register_session_tools(
    ...
    """Register v1 session/cognitive state MCP tools."""
    ...
    @server.tool(
        ...
        description="Start a new session or restore "
        "existing one. Required before adding facts.",
    )
    async def rlm_start_session(
        ...
        try:
            ...
            return {
                ...
                "message": (f"Session started: " f"{state.session_id}"),
            }
    ...
    @server.tool(
        ...
        description="Auto-restore session and return "
        "context for prompt injection. "
        "One-call solution for always having project "
        "knowledge.",
    )
    async def rlm_auto_context(
        ...
        try:
            ...
            compact = manager.get_state_for_injection(
    ...
    async def rlm_delete_session(
        ...
        try:
            ct = manager.storage.delete_session(session_id)
            return {
                "status": "success",
                "session_id": session_id,
                "deleted_versions": ct,
                "message": (
                    f"Deleted {ct} version(s)" if ct > 0 else "Session not found"
                ),
            }
        except Exception as e:
            return {"status": "error", "message": str(e)}
    ...
    async def rlm_get_audit_log(
        ...
        try:
            entries = manager.storage.get_audit_log(
                session_id,
                limit,
            )
            return {
