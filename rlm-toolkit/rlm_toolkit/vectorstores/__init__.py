...
class VectorStore(ABC):
    ...
    def from_documents(
        ...
        **kwargs,
        ...
        Convenience constructor that creates the store and adds documents
        in one call — the pattern used throughout RLM-Toolkit docs.

        Args:
            documents: List of Document objects with .content and .metadata.
            embedding_function: Embedding function or model instance.
            **kwargs: Passed to the subclass __init__.

        Returns:
            A new VectorStore instance with documents already indexed.
        """
...
class WeaviateVectorStore(VectorStore):
        ...
        if self._client is None:
            try:
                ...
                self._client = weaviate.Client(
                    url=self.url,
                    auth_client_secret=(
                        weaviate.AuthApiKey(api_key=self.api_key)
                        if self.api_key
                        else None
                    ),
                )
    ...
    def similarity_search(
        ...
        result = (
            client.query.get(self.class_name, ["text"])
        ...
        return [
            (item.get("text", ""), item.get("_additional", {}).get("distance", 0))
            for item in items
        ]
